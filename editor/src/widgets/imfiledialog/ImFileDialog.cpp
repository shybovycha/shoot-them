...
namespace ifd
{
    ...
    bool FolderNode(const char* label, ImTextureID icon, bool& clicked)
    {
        ...
        // Icon, text
        float icon_posX = pos.x + g.FontSize + g.Style.FramePadding.x;
        float icon_posY = pos.y + (ICON_SIZE / 2) - (g.FontSize / 3) + g.Style.FramePadding.y;
        ...
        window->DrawList->AddImage(icon, ImVec2(icon_posX, icon_posY), ImVec2(icon_posX + ICON_SIZE, icon_posY + ICON_SIZE));
    ...
    void FileDialog::m_renderFileDialog()
    {
        ...
        auto escapeKey = static_cast<ImGuiKey>(ImGui::GetIO().KeyMap[ImGuiKey_Escape]);
        if (ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && escapeKey >= 0 && ImGui::IsKeyPressed(escapeKey)) {
            std::cout << "window focused & escape pressed, hiding\n";
            m_isOpen = false;
        }
    }
}// namespace ifd
